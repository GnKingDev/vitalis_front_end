# Routes Backend - Historique des Demandes d'Imagerie

## Vue d'ensemble

Ce document décrit les routes backend pour récupérer l'historique des demandes d'imagerie d'un patient. Ces routes sont utilisées dans la page de consultation du médecin pour afficher l'historique des demandes précédentes.

## Route principale

### `GET /api/v1/imaging/requests`

Récupère la liste des demandes d'imagerie avec possibilité de filtrer par patient.
...
```bash
GET /api/v1/imaging/requests?patientId=123e4567-e89b-12d3-a456-426614174000&limit=50
```
...
if (req.query.search) {
  whereClause[Op.or] = [
    { '$ImagingExam.name$': { [Op.iLike]: `%${req.query.search}%` } }
  ];
}
```
...
```javascript
const requests = await ImagingRequest.findAndCountAll({
  where: whereClause,
  include: [
    {
      model: Patient,
...
- **Lab** : Peut voir toutes les demandes (pour traitement, car le labo gère aussi l'imagerie)
...
const checkImagingRequestAccess = async (req, res, next) => {
...
Le frontend utilise cette route pour afficher l'historique des demandes dans un modal lorsque l'utilisateur clique sur le bouton "Voir l'historique" dans l'onglet Imagerie de la page de consultation.

```typescript
// Exemple d'utilisation dans ConsultationPage.tsx
const loadImagingHistory = async () => {
...
Après la création d'une nouvelle demande d'imagerie, l'historique est automatiquement rechargé pour afficher la nouvelle demande.
...
- `ImagingRequest.belongsToMany(ImagingExam)` via `ImagingRequestExam`
- `ImagingRequest.belongsTo(Patient)`
- `ImagingRequest.belongsTo(User, { as: 'doctor' })`
- `ImagingRequest.belongsTo(Consultation)`
- `ImagingRequest.hasOne(ImagingResult)`
...
5. **Statuts** : Les statuts possibles sont :
   ...
   - `in_progress` : En cours de traitement (imagerie)
   - `completed` : Résultats disponibles
   - `cancelled` : Demande annulée

6. **Gestion par le laboratoire** : Notez que dans ce système, le département "Labo" est également responsable du traitement des demandes d'imagerie, d'où la permission `lab` pour voir toutes les demandes.

## Exemple de route Express.js

```javascript
router.get('/imaging/requests', authenticateToken, checkImagingRequestAccess, async (req, res) => {
  try {
    const page = parseInt(req.query.page) || 1;
    const limit = Math.min(parseInt(req.query.limit) || 10, 100);
    const offset = (page - 1) * limit;
    ...
    const { count, rows: requests } = await ImagingRequest.findAndCountAll({
      where: whereClause,
      include: [
        {
          model: Patient,
